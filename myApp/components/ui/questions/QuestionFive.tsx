...
export default function QuestionFive() {
  ...
  const setQuestionFiveAnswer = useAnswerStore((s) => s.setQuestionFiveAnswer);
  
  const handleContinue = () => {
    if (value) {
      // Save the answer to the store
      setQuestionFiveAnswer(value);
      // Navigate to next question
      setOnboardingStage('questionSix');
    }
  };
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
          <View style={styles.contentWrapper}>
            <View style={styles.headerSection}>
              <LinearGradient colors={['#F87171', '#FB923C']} style={styles.iconContainer}>
                <MaterialCommunityIcons name="alert-circle-outline" size={40} color="white" />
              </LinearGradient>
              <Text style={styles.title}>Coping with substances?</Text>
              <Text style={styles.subtitle}>Do you sometimes use alcohol, smoking, etc. to cope with stress?</Text>
            </View>
              ...
              {OPTIONS.map((o) => {
                ...
                return (
                  <Pressable key={o.id} style={styles.optionWrapper} onPress={() => setValue(o.id)}>
                    {({ pressed }) => (
                      <LinearGradient
                        colors={isActive ? ['#F87171', '#FB923C'] : ['#ffffff', '#ffffff']}
            ...
            <Pressable disabled={disabled} style={styles.buttonWrapper} onPress={handleContinue}>
              {({ pressed }) => (
                <LinearGradient colors={['#F87171', '#FB923C']} style={[styles.primaryButton, (pressed || disabled) && styles.primaryButtonPressed]}>
