...
const OPTIONS = [
  { id: 'restful', label: 'Restful and regular' },
  { id: 'irregular', label: 'Irregular, not enough sleep' },
  { id: 'insomnia', label: 'Trouble falling asleep / insomnia' },
];
...
export default function QuestionTwo() {
  ...
  const setQuestionTwoAnswer = useAnswerStore((s) => s.setQuestionTwoAnswer);
  ...
  const handleContinue = () => {
    if (value) {
      ...
      setOnboardingStage('questionThree');
    }
  };
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
          <View style={styles.contentWrapper}>
            <View style={styles.headerSection}>
              <LinearGradient colors={['#A78BFA', '#F472B6']} style={styles.iconContainer}>
                <MaterialCommunityIcons name="moon-waxing-crescent" size={40} color="white" />
              </LinearGradient>
              <Text style={styles.title}>How's your sleep routine?</Text>
              <Text style={styles.subtitle}>Sleep quality affects your mental wellbeing significantly.</Text>
            </View>
              ...
              {OPTIONS.map((o) => {
                ...
                return (
                  <Pressable key={o.id} style={styles.optionWrapper} onPress={() => setValue(o.id)}>
                    {({ pressed }) => (
                      <LinearGradient
                        colors={isActive ? ['#A78BFA', '#F472B6'] : ['#ffffff', '#ffffff']}
            ...
            <Pressable disabled={disabled} style={styles.buttonWrapper} onPress={handleContinue}>
              {({ pressed }) => (
                <LinearGradient colors={['#A78BFA', '#F472B6']} style={[styles.primaryButton, (pressed || disabled) && styles.primaryButtonPressed]}>
