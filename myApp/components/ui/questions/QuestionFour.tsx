...
export default function QuestionFour() {
  ...
  const setQuestionFourAnswer = useAnswerStore((s) => s.setQuestionFourAnswer);
  
  const handleContinue = () => {
    if (value) {
      // Save the answer to the store
      setQuestionFourAnswer(value);
      // Navigate to next question
      setOnboardingStage('questionFive');
    }
  };
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
          <View style={styles.contentWrapper}>
            <View style={styles.headerSection}>
              <LinearGradient colors={['#34D399', '#10B981']} style={styles.iconContainer}>
                <MaterialCommunityIcons name="heart-pulse" size={40} color="white" />
              </LinearGradient>
              <Text style={styles.title}>Do you exercise regularly?</Text>
              <Text style={styles.subtitle}>Physical activity can greatly impact your mental health.</Text>
            </View>
              ...
              {OPTIONS.map((o) => {
                ...
                return (
                  <Pressable key={o.id} style={styles.optionWrapper} onPress={() => setValue(o.id)}>
                    {({ pressed }) => (
                      <LinearGradient
                        colors={isActive ? ['#34D399', '#10B981'] : ['#ffffff', '#ffffff']}
            ...
            <Pressable disabled={disabled} style={styles.buttonWrapper} onPress={handleContinue}>
              {({ pressed }) => (
                <LinearGradient colors={['#34D399', '#10B981']} style={[styles.primaryButton, (pressed || disabled) && styles.primaryButtonPressed]}>
